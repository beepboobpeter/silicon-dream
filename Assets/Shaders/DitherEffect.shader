Shader "Custom/LitRetroDither_FullGradient"
{
    Properties
    {
        ...
        _BendStrength ("Bend Strength",Range(0,0.1)) = 0.05
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // This is where you could add reusable modules
            // #pragma multi_compile __ BEND_ON
            // #include "Assets/Shaders/BendRadial.hlsl"
            ...
            float3 ApplyCameraBend(float3 positionOS)
            {
                // Convert object → world
                float3 worldPos = TransformObjectToWorld(positionOS);

                // Step 1: subtract camera position
                float3 offset = worldPos - _WorldSpaceCameraPos;

                // Step 2 + 3: square the Z distance
                float bend = offset.z * offset.z;

                // Step 4: multiply by negative variable
                bend *= -_BendStrength;

                // Step 5: create offset vector affecting Y
                float3 bendOffset = float3(0, bend, 0);

                // Step 6: add to world position
                worldPos += bendOffset;

                // Step 7: convert world → object space
                float3 newObjectPos = TransformWorldToObject(worldPos);

                // Step 8: return modified vertex
                return newObjectPos;
            }
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                if (quant <= 0.5)
                {
                    ...
                    finalColor = lerp(
                        _ShadowColor.rgb,
                        _MidColor.rgb * albedo.rgb,
                        t
                    );
                }
                else
                {
                    float t = (quant - 0.5) / 0.5;
                    finalColor = lerp(
                        _MidColor.rgb * albedo.rgb,
                        _LightColor.rgb * albedo.rgb,
                        t
                    );
                }
